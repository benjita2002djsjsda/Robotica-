...
pub fn evaluar_robustez(
    ...
    for (f64Izq, f64Centro, f64Der) in ARR_TPL_F64X3_MODELOS_RUIDO.iter() {
        ...
        let uiCambios = ref_hm_s_sPoliticaBase
            .iter()
            .filter(|(sStateKey, sOriginalActionValue)| { // sStateKey: &&String, sOriginalActionValue: &&String
                match hm_s_sPoliticaAdaptada.get(**sStateKey) { // get expects &String or String, so dereference sStateKey twice
